...
- **Played together**: double stops in 3rds, 4ths, 5ths or 6ths, and the harmonized scale as triads, 7th chords or quartal voicings — diatonic, so the quality follows the key rather than moving one shape up the neck. Every grip stays inside three frets; where close position won't fit a hand, a voice drops an octave, which is how these are played anyway
